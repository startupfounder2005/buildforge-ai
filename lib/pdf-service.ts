...
export async function generateLegalPDF(data: DocumentData, isDraft: boolean = true): Promise<Uint8Array> {
    ...
    // Load Logo
    let logoImage: import('pdf-lib').PDFImage | undefined
    try {
        const logoPath = path.join(process.cwd(), 'public', 'obsidian-logo.png')
        // fs is imported from 'fs/promises'
    ...
    if (data.category === 'safety') {
        ...
        const drawHeader = (currPage: PDFPage) => {
            const { width, height } = currPage.getSize()
            // Hexagon Icon (Small) -> Replaced with Image
            if (logoImage) {
                currPage.drawImage(logoImage, {
                    x: margin,
                    y: height - 50,
                    width: 30, // Smaller for contract header
                    height: 30
                })
            } else {
        ...
        const drawHeader = (currPage: PDFPage) => {
            ...
            if (logoImage) {
                currPage.drawImage(logoImage, {
                    x: margin,
                    y: height - 60, // Adjusted Y for larger logo
                    width: 40,
                    height: 40
                })
            } else {
            ...
            // Was x: width - margin - 140 (approx). Let's align cleanly to right margin.
            const dateText = `DATE: ${data.date_created || 'Pending'}`;
            const validText = `VALID UNTIL: ${data.expiration_date || '30 Days'}`;

            const dateW = fontBold.widthOfTextAtSize(dateText, 9);
            const validW = fontRegular.widthOfTextAtSize(validText, 9);

            currPage.drawText(dateText, { x: width - margin - dateW, y: height - 50, size: 9, font: fontBold })
            currPage.drawText(validText, { x: width - margin - validW, y: height - 62, size: 9, font: fontRegular, color: rgb(0.8, 0, 0) })
        }
        ...
        // --- HEADER SECTION ---
        // --- HEADER SECTION ---
        // Hexagon Logo (Replaced with Image)
        if (logoImage) {
            page.drawImage(logoImage, {
                x: margin,
                y: yCursor, // Image draws from bottom-left corner? No, y is usually bottom-left. 
                // Wait, drawImage(img, {x, y, width, height}). 
                // SVG was: y: yCursor, scale 0.8. SVG path coords were 0-60. 
                // Let's assume height ~60.
                width: 60,
                height: 60
            })
        } else {
        ...
        if (yCursor - sigH < margin) {
            ...
            yCursor = height - margin
            if (isDraft) {
                page.drawText("MOCK ONLY - DRAFT - GENERATED BY OBSIDIAN", {
                    x: margin - 70, y: 150, size: 72, font: fontBold, color: rgb(1, 0, 0), opacity: 0.12, rotate: degrees(45)
                })
            }
        }
